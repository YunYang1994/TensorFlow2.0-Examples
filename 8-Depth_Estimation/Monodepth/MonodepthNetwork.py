...
class upsample_conv(tf.keras.Model):
    def __init__(self, filters, kernel_size, scale):
        ...
        self.conv = tf.keras.Sequential([tf.keras.layers.ZeroPadding2D(1),
                tf.keras.layers.Conv2D(filters, kernel_size, strides=1, padding="valid", activation="relu")]
                )
...
class depth_decoder(tf.keras.Model):
    ...
    def call(self, input_features, training=False):
        ...
        self.disp3 = 0.3 * self.disp_layers[3](iconv3) # H/8 - 2D
        print(self.disp3.shape)
        up_disp3 = upsample_nn(self.disp3, 2)
        ...
        self.disp2 = 0.3 * self.disp_layers[2](iconv2) # H/4 - 2D
        print(self.disp2.shape)
        up_disp2 = upsample_nn(self.disp2, 2)
        ...
        self.disp1 = 0.3 * self.disp_layers[1](iconv1) # H/2 - 2D
        print(self.disp1.shape)
        up_disp1 = upsample_nn(self.disp1, 2)

        upconv0 = self.upsample_conv_layers[0](iconv1)
        concat0 = tf.concat([upconv0, up_disp1], axis=3)
        iconv0  = self.conv_layers[0](concat0)
        self.disp0 = 0.3 * self.disp_layers[0](iconv0) # H - 2D
        print(self.disp0.shape)
        return self.disp0, self.disp1, self.disp2, self.disp3
